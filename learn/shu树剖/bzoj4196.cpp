...
namespace shupou{
	...
	{
		if(!pac)return;
		segtree::update(segtree::root,1,n,id[shang[pac]],id[pac],1);
		ins(fa[shang[pac]]);
	}
	int install(int pac)
	{
	...
	int uninstall(int pac)
	{
		int qian=segtree::ask(segtree::root,1,n,1,n);
		out();
		segtree::update(segtree::root,1,n,id[pac],id[pac]+sz[pac]-1,0);
		out();
...
int main(int argc, char const *argv[])
{
	...
	dfs2(1,1);
	for(int a=0;a<n;a++) printf("%d ",a); putchar('\n');
	for(int a=0;a<n;a++) printf("%d ",shang[a]); putchar('\n');
	for(int a=0;a<n;a++) printf("%d ",xia[a]); putchar('\n');
	for(int a=0;a<n;a++) printf("%d ",id[a]); putchar('\n');
	int m; read(m);
	while(m--)
	{
		int opt;
